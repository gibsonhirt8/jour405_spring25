...
spotify_songs |> 
  ...
  geom_histogram(aes(x = danceability)) +
  geom_vline(aes(xintercept = mean(danceability)), color = "red", linetype = "dashed") +
  geom_vline(aes(xintercept = mean(danceability) - sd(danceability)), color = "blue", linetype = "dashed") +
  geom_vline(aes(xintercept = mean(danceability) + sd(danceability)), color = "blue", linetype = "dashed")
```
...
After examining the danceability scores from this dataset, I noticed that the data is slightly skewed to the left, meaning that most of the data is clustered to the right of the mean. The mean and standard deviation tells me that on average, songs on Spotify tend to lean towards having a strong danceability score, as one standard deviation to the left of the mean is still above a score of 0.5. Assuming the 0.5 is a completely middle of the pack score, over 75% of the data has a danceability score above 0.5, meaning most songs can be dance to.

Story Pitch:

If I was writing a story about this data, I would definitely talk about how in general, more than 75% of songs had scores that would make them be considered danceable. I would also make sure to mention that 10 out of the 11 songs with the highest danceability scores are typically referred to as rap songs. This definitely caught my eye because when I think of songs that are easy to dance to, rap is never the genre that comes to mind first.
