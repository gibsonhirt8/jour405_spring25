...
The two variables that seem to have the strongest correlation with are pct_white and pct_unemployed. The bachelor's degree category seems to have a low correlation with voter turnout. The correlation with population is a little bit stronger than with bachelor's degree but still no where close to the white or unemployed variables. Looking at the scatterplots confirms my original observations for the most part. I think it's fair to say of the scatterplots except population have at least a small linear relationship but it is clear that white and unemployed have the strongest ones.
...
In general, this model seems to do a decent job at predicting voter turnout. The adjusted r-squared value is 0.8023, meaning 80% of the change in voter turnout is caused by the independent variables. Income, median age and percent white are statistically significant predictors while population, education level and unemployment are not. There are enough variables that are significant to make this model useful for predicting voter turnout.
...
Of the four models I now have, three of them have very similar adjusted r-squared values. Model 2 is about 0.02 higher than 1 and 3, but all three are significantly higher than model 4 that has an adjusted r-squared value of 0.746 while the other three are all around 0.80. Median age was the big difference between models 2 through 4, so it seems that variable might not be the best predictor of voter turnout.
...
I could use this analysis to help identify which community characteristics are linked to higher or lower voter turnout. For example, if the data said people with an income above a certain level are more likely to vote, I could write a story that talks about how this will effect a local election where I know the demographics of the voters. This analysis also allows me to compare different locations throughout the country to determine where efforts need to be spent to increase voter turnout.
